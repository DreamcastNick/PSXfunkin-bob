...
void Back_Week9_Window(Back_Week9 *this)
{
...
void Back_Week9_DrawBG(StageBack *back)
{
	...
	if (stage.note_scroll >= 0 && (stage.flag & STAGE_FLAG_JUST_STEP) && (stage.song_step & 0xF) == 0)
		Back_Week9_Window(this);
	
	//Draw rooftop
	fx = stage.camera.x;
	fy = stage.camera.y;
	
	static const struct Back_Week9_RoofPiece
	{
		RECT src;
		fixed_t scale;
	} roof_piece[] = {
	...
	const struct Back_Week9_RoofPiece *roof_p = roof_piece;
