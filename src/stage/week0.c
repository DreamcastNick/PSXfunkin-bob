...
void Back_Week0_Window(Back_Week0 *this)
{
...
void Back_Week0_DrawBG(StageBack *back)
{
	...
	if (stage.note_scroll >= 0 && (stage.flag & STAGE_FLAG_JUST_STEP) && (stage.song_step & 0xF) == 0)
		Back_Week0_Window(this);
	
	//Draw rooftop
	fx = stage.camera.x;
	fy = stage.camera.y;
	
	static const struct Back_Week0_RoofPiece
	{
		RECT src;
		fixed_t scale;
	} roof_piece[] = {
	...
	const struct Back_Week0_RoofPiece *roof_p = roof_piece;
...
StageBack *Back_Week0_New(void)
{
	...
	this->back.draw_bg = Back_Week0_DrawBG;
